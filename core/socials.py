"""Brand social icons for the footer — inline SVG marks with hover gradients.

Kept apart from core/icons.py (Material Symbols) because these are brand
marks rather than UI glyphs. Gradients are defined inside each inline SVG
and applied on hover via a CSS custom property so the icons rest muted and
only take on their brand colour/gradient when hovered.
"""
...
_ICONS = {
    ...
    "Instagram": {
        "slug": "instagram",
        "path": (
            "M7.0301.084c-1.2768.0602-2.1487.264-2.911.5634-.7888.3075-1.4575.72-2.1228 1.3877-.6652.6677-1.075 "
            "1.3368-1.3802 2.127-.2954.7638-.4956 1.6365-.552 2.914-.0564 1.2775-.0689 1.6882-.0626 4.947.0062 "
            "3.2586.0206 3.6671.0825 4.9473.061 1.2765.264 2.1482.5635 2.9107.308.7889.72 1.4573 1.388 2.1228.6679.6655 "
            "1.3365 1.0743 2.1285 1.38.7632.295 1.6361.4961 2.9134.552 1.2773.056 1.6884.069 4.9462.0627 3.2578-.0062 "
            "3.668-.0207 4.9478-.0814 1.28-.0607 2.147-.2652 2.9098-.5633.7889-.3086 1.4578-.72 2.1228-1.3881.665-.6682 "
            "1.0745-1.3378 1.3795-2.1284.2957-.7632.4966-1.636.552-2.9124.056-1.2809.0692-1.6898.063-4.948-.0063-3.2583-.021-3.6668-.0817-4.9465-.0607-1.2797-.264-2.1487-.5633-2.9117-.3084-.7889-.72-1.4568-1.3876-2.1228C21.2982 "
    ...
    "X / Twitter": {
        "slug": "x",
        "path": (
            "M14.234 10.162 22.977 0h-2.072l-7.591 8.824L7.251 0H.258l9.168 13.343L.258 24H2.33l8.016-9.318L16.749 "
    ...
    "Email": {
        "slug": "gmail",
        "path": (
            "M24 5.457v13.909c0 .904-.732 1.636-1.636 1.636h-3.819V11.73L12 16.64l-6.545-4.91v9.273H1.636A1.636 "
...
def social_row() -> str:
    ...
    for name, url in SOCIAL_LINKS.items():
        icon = _ICONS[name]
        gid = f"ls-grad-{icon['slug']}"
        stops = "".join(
            f'<stop offset="{offset}%" stop-color="{color}"/>' for color, offset in icon["stops"]
        )
        anchors.append(
            f'<a class="social-link" href="{url}" target="_blank" rel="noopener" '
            f'aria-label="{name}" title="{name}" style="--g: url(#{gid})">'
    ...
    return f'<div class="social">{" ".join(anchors)}</div>'
